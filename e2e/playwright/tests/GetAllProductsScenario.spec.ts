...
test.describe('Product API', () => {
  ...
  test('Get all products', async ({ request }) => {
    ...
    const response = await request.get(`${baseURL}${apiData.URL}`, {
      headers: apiData.REQ_HEADERS ? JSON.parse(apiData.REQ_HEADERS) : undefined
    });

    expect(response.status()).toBe(apiData.RESPONSE_CODE);
    const responseBody = await response.json();
    expect(responseBody).toEqual(JSON.parse(apiData.RESPONSE_BODY));
  });
});
