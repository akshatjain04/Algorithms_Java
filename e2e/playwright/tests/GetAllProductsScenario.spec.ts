...
beforeAll(async () => {
  try {
    testData = await parseCSVFile('undefinede2e\\playwright\\tests\\GetAllProductsScenario.spec.csv');
...
test.describe('Product API - Get all products', () => {
  test('should return an empty list of products', async ({ request }) => {
    const apiSpec = testData.find(spec => spec.METHOD === 'GET' && spec.URL.includes('/api/products'));
    const response = await request.get(apiSpec.URL, {
      headers: apiSpec.REQ_HEADERS ? JSON.parse(apiSpec.REQ_HEADERS) : undefined
    });
    expect(response.status()).toBe(apiSpec.RESPONSE_CODE);
    const responseBody = await response.json();
    expect(responseBody).toEqual(JSON.parse(apiSpec.RESPONSE_BODY));
  });
});
